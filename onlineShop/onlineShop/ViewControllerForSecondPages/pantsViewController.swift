...
class pantsViewController: UIViewController {
    var segmentName4 = UISegmentedControl()
    var pantsName = ["1", "2", "3", "4", "5"]
    var pantsImage = UIImageView()
    var pantsImagesArray = [UIImage(named: "pant"),UIImage(named: "pants2"), UIImage(named: "pants3"), UIImage(named: "pants4"), UIImage(named: "pants5")]
    ...
    override func viewDidLoad() {
        ...
        /// images set
        self.pantsImage.frame = CGRect(x: 50, y: 180, width: 300, height: 250)
        self.pantsImage.image = pantsImagesArray [0]
        self.view.addSubview(pantsImage)
        // segment
        self.segmentName4 = UISegmentedControl(items: pantsName)
        self.segmentName4.frame = CGRect(x: 90, y: 450, width: 200, height: 40)
        self.view.addSubview(segmentName4)
        self.segmentName4.addTarget(self, action: #selector(selectImage4), for: .valueChanged)
        self.segmentName4.addTarget(self, action: #selector(selectPrice), for: .valueChanged)
    ...
    @objc func selectImage4 (target : UISegmentedControl) {
        if target == self.segmentName4 {
            let segmentIndex = target.selectedSegmentIndex
            self.pantsImage.image = pantsImagesArray[segmentIndex]
        }
    }
...
extension pantsViewController :  UIPickerViewDelegate, UIPickerViewDataSource {
